...
CLASS_COLORS = {0:[0,0,0], 1:[1,1,0], 2:[1,0.5,0], 3:[0,0,1], 4:[0,1,0], 5:[0,1,1], 6:[1,0.4,1], 7:[1,0,0]}
...
def show_img_preds(img, depth_pred, seg_pred, uc_th=0.0, apply_depth_mask=False):  
    ...
    plt.title('masked de')
    if apply_depth_mask:
        # mask high gradient regions ~ these are usally not as accurate
        grad = np.asarray(np.gradient(depth_pred))
        grad = np.abs(grad[0,:,:]) + np.abs(grad[1,:,:])
        grad_mask = grad < 0.9
        
        depth_mask = depth_pred < 50.0 # mask everything thats farther than 50m     
...
def show_point_cloud(img, depth_pred, seg_pred, f_len, uc_th=0.0, apply_depth_mask=False):
    ...
    if apply_depth_mask:
        ...
        depth_mask = (depth_pred < 50.0)*(depth_pred > 5.0) # mask everything thats farther than 50m
